...
const { verifyAuditLog, logAction } = require("../services/auditService");
...
router.get("/", authMiddleware , permissionMiddleware(PERMISSIONS.VERIFY_AUDIT), async (req, res) => {
    try {
...
router.get("/report", authMiddleware , permissionMiddleware(PERMISSIONS.REPORT_GENERATE), async (req, res) => {
    let report;
    try {
...
router.get("/report/:reportId/download", authMiddleware , permissionMiddleware(PERMISSIONS.REPORT_DOWNLOAD), async (req, res) => {
    let report;
    try {
...
router.post("/report/upload", authMiddleware , permissionMiddleware(PERMISSIONS.REPORT_VALIDATE), upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
...
router.get("/keys/public/fingerprint", authMiddleware , async (req, res) => {
    try {
...
router.get("/reports", authMiddleware, permissionMiddleware(PERMISSIONS.REPORT_DOWNLOAD), async (req, res) => {
    try {
