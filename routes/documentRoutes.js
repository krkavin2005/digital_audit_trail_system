...
const { canDelete, canTransition } = require("../workflow/documentWorkflow");
const Notification = require("../models/Notification");

router.post("/upload", authMiddleware , permissionMiddleware(PERMISSIONS.FILE_UPLOAD), docupload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
...
router.get("/", authMiddleware , permissionMiddleware(PERMISSIONS.FILE_VIEW), async (req, res) => {
    try {
...
router.get("/:documentId", authMiddleware, permissionMiddleware(PERMISSIONS.FILE_VIEW), async (req, res) => {
    try {
...
router.delete("/:documentId", authMiddleware , permissionMiddleware(PERMISSIONS.FILE_DELETE), async (req, res) => {
    try {
        const { documentId } = req.params;
        const document = await Document.findOne({ documentId, isDeleted: false });
...
router.patch("/:documentId/transition", authMiddleware , async (req, res) => {
    try {
        const { documentId } = req.params;
...
router.get("/:documentId/history", authMiddleware, permissionMiddleware(PERMISSIONS.FILE_VIEW), async (req, res) => {
    try {
        const { documentId } = req.params;
